...
class LaporanPenerimaan extends StatefulWidget {
  ...
  LaporanPenerimaan({required this.viewModel, required this.salesmanData});

  @override
  _LaporanPenerimaanState createState() => _LaporanPenerimaanState();
}

class _LaporanPenerimaanState extends State<LaporanPenerimaan> {
  List<TransaksiPenerimaan> data = [];
  ...
  Future<void> _fetchBarangs() async {
    try {
      await widget.viewModel.getLaporanPenerimaan(
          widget.salesmanData['ID_SALES'], dateController.text);
      setState(() {
        data = List.from(widget.viewModel.laporanPenerimaan);
      });

      print (data);
    } catch (error) {
      setState(() {
        data = []; // Atur data ke list kosong jika terjadi kesalahan
      });
    }
  }
        ...
        children: [
          ...
          Expanded(
            child: data.isEmpty || data == null
                ? Center(
                    child: Text(
                      'Tidak ada penerimaan di tanggal ini',
                      ...
                      return Theme(
                        data: Theme.of(context).copyWith(
                          visualDensity: VisualDensity
                              .compact, // Menghapus padding atas pada ListTile
                        ),
                        child: ListTile(
                          ...
                          subtitle: ListView(
                            ...
                            // Untuk memastikan ListView tidak scroll di dalam ListView
                            children: transaksisSama.map((transaksi) {
                              return ListTile(
                                contentPadding: EdgeInsets.only(
                                    top: 0, left: 16, right: 16, bottom: 0),
                                horizontalTitleGap: 0,
                                // Menghapus jarak horizontal antara title dan leading/trailing widget
                                minVerticalPadding: 0,
                                // Menghapus jarak vertikal minimum
                                minLeadingWidth: 0,
                                // Menghapus lebar minimum leading widget
                                title: Text(
                                  '${transaksi.idBarang} - ${transaksi.nama}',
                                  style: TextStyle(
                                    fontSize:
                                        30, // Mengatur ukuran teks menjadi 30
                                    color: Color(
                                        0xFF120DF4), // Mengatur warna teks menjadi 120DF4
                                  ),
                                ),
                                trailing: Text(
